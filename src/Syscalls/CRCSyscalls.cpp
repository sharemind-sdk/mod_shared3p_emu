                                 ...
                                 returnValue, c)
{
    ...
    try {
        Additive3ppPDPI & pdpi =
                *static_cast<Additive3ppPDPI *>(handles.pdpiHandle);
        ...
        if (!pdpi.isValidHandle<a3pp_xor_uint8_t>(inputVecHandle) ||
            !pdpi.isValidHandle<value_t>(outputHandle))
        {
            return SHAREMIND_MODULE_API_0x1_GENERAL_ERROR;
        }

        const share_vec<a3pp_xor_uint8_t> & inputVec = *static_cast<share_vec<a3pp_xor_uint8_t> *>(inputVecHandle);
        ...
        /// \todo
        //if (! CRCProtocol<mode>(pdpi).invoke (inputVec, output[0])) {
        //    return SHAREMIND_MODULE_API_0x1_GENERAL_ERROR;
        //}
